...
data_set <- read.csv("my_updated_dataset.csv")
...
y_max <- max(college_summary$Players)
...
# Pivot to wide format for numerical display (schools as rows, year ranges as columns)
college_summary_wide <- college_summary %>%
  pivot_wider(names_from = YearRange, values_from = Players, values_fill = 0) %>%
  arrange(School)  # Sort by school name for better readability

# Print the numerical data as a table
