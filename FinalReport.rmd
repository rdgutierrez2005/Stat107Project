...
Github Link : https://github.com/rdgutierrez2005/Stat107Project-The3rdTry

```{r}
# This chunk loads the requirements script to attach necessary packages like tidyverse and gridExtra.   It sets up the environment for data manipulation and visualization.
...
# This chunk sources the data cleaning script to process the raw dataset. It reads the     #            "College_Dataset.xlsx" file, renames columns, extracts start years, filters to 2011+, selects key     variables, cleans text, and saves the cleaned data.
# Outcome: A cleaned dataset is created and saved as "cleaned_data.RDS" for use in later chunks.
...
```{r}
# This chunk sources the plotting script to create bar plots. It generates individual bar charts for    each SEC school, showing player counts across the three year ranges.
# Outcome: Bar plots are produced and displayed, visualizing trends in player representation.
source("02_funct_Plots.R")
```

```{r}
# This chunk sources the table script to summarize data numerically. It creates a tibble table with     player counts by school and year range.
